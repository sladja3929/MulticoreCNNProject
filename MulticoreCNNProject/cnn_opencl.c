...
void cnn_init() {
	...
	/*softmax_kernel = clCreateKernel(softmax_program, "softmax", &err);    CHECK_ERROR(err);*/

	buf1 = clCreateBuffer(context, CL_MEM_READ_WRITE, sizeof(cl_float) * (batch_num * 65536), NULL, &err);    CHECK_ERROR(err);
	buf1_1 = clCreateBuffer(context, CL_MEM_READ_WRITE, sizeof(cl_float) * (batch_num * 65536), NULL, &err);    CHECK_ERROR(err);
...
static void pooling_layer(float* inputs, float* outputs, int d, int n) {
	...
	err = clSetKernelArg(pooling_kernel, 2, sizeof(cl_int), &n);	CHECK_ERROR(err);
...
static void fc_layer(float* input_neuron, float* output_neuron, float* weights, float* biases, int M, int N) {
	...
	err = clEnqueueWriteBuffer(kernel_queue, buf3, CL_TRUE, 0, sizeof(cl_float) * M * N, weights, 0, NULL, NULL);
	CHECK_ERROR(err);

	err = clEnqueueWriteBuffer(kernel_queue, buf4, CL_TRUE, 0, sizeof(cl_float) * M, biases, 0, NULL, NULL);
	CHECK_ERROR(err);
...
//static void softmax(float* output, int* labels, float* confidences, int N, int i) {
//	size_t global_size = PARALLEL * N;
//	size_t local_size = N;
//
//	err = clSetKernelArg(softmax_kernel, 0, sizeof(cl_mem), &buf2);
//	CHECK_ERROR(err);
//
...
//	CHECK_ERROR(err);
//
//	err = clEnqueueNDRangeKernel(kernel_queue, softmax_kernel, 1, NULL, &global_size, &local_size, 0, NULL, NULL);
//	CHECK_ERROR(err);
//
//	err = clEnqueueReadBuffer(kernel_queue, buf2, CL_TRUE, 0, sizeof(cl_float) * N * PARALLEL, output, 0, NULL, NULL);
//	CHECK_ERROR(err);
//
//	clFinish(kernel_queue);
//}
